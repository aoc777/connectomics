...
from connectomics.data.dataset import build_dataloader, collate_fn_test, get_dataset
...
from .utils import all_reduce_mean, MAELoss
from engine.build_seg_model import UNETR
from .mae import *

class TrainerMAE(Trainer):
    def __init__(self,
                 cfg: CfgNode,
                 device: torch.device,
                 mode: str = 'train',
        ...
        self.dataloader = build_dataloader(self.cfg, None, self.mode, rank=rank, cf=collate_fn_test)
        self.dataloader = iter(self.dataloader)
        self.loss_scaler = NativeScaler()
        self.accum_iter = self.cfg.MAE.ACCUM_ITER
    ...
    def train(self):
        ...
        for i in range(self.total_iter_nums):
            ...
            # if (iter_total + 1) % self.accum_iter == 0:
            #     self.scheduler_step(iter_total, None)
            ...
            pred, mask = self.model(inputs, self.cfg.MAE.MASKRATIO)
            loss = self.criterion(inputs, pred, mask, self.model.module)

            self._train_misc(loss, pred, mask, inputs, iter_total)

    def _train_misc(self, loss, pred, mask, inputs, iter_total):
        # loss /= self.accum_iter
        # self.loss_scaler(loss, self.optimizer, parameters=self.model.parameters(),
        #                  update_grad=(iter_total + 1) % self.accum_iter == 0)
        #
        # if (iter_total + 1) % self.accum_iter == 0:
        #     self.optimizer.zero_grad()
        ...
        losses_vis = {'MAEloss': loss,}
        # logging and update record
        if hasattr(self, 'monitor'):
            # import pdb
            # pdb.set_trace()
            do_vis = self.monitor.update(iter_total, loss, losses_vis, self.optimizer.param_groups[0]['lr'])
            # visualize inputs and reconstructions
            if do_vis:
                mask = mask.detach()
                mask = mask.unsqueeze(-1).repeat(1, 1, self.model.module.patch_embed.patch_size[1] ** 2 * 1)  # (N, H*W, p*p*3)
                mask = self.model.module.unpatchify(mask)
                pred = (self.model.module.unpatchify(pred) + 1) / 2
                inputs = (inputs + 1) / 2
                im_mask = inputs * (1 - mask)
                im_paste = inputs * (1 - mask) + (pred * mask)

                image_groups = {"inputs_recons": [inputs,
                                                  im_mask,
                                                  pred,
                                                  im_paste]}
                self.monitor.visualize_image_groups(iter_total, image_groups, is_3d=True)
        ...
        if self.is_main_process:
            ...
            info = [
                ...
                'Lr %.7f' % self.optimizer.param_groups[0]['lr'],
                'Loss: %.4f,' % loss.item(),
                'Time Left %.2fh.' % est_time_left]
            print(' '.join(info))

        del pred, inputs, loss,
    ...
    def build_mae(self):
        ...
        return make_parallel(model, self.cfg, self.device, self.rank, find_unused_parameters=True)

class TrainerftMAE(Trainer):
    def __init__(self,
                 cfg: CfgNode,
                 device: torch.device,
                 mode: str = 'train',
                 ...
                 checkpoint: Optional[str] = None):
        self.init_basics(cfg, device, mode, rank)
        self.device_id = list(range(cfg.SYSTEM.NUM_GPUS))
        self.model = make_parallel(UNETR(pretrain=self.cfg.MAE.PRETRAIN,output_dim=self.cfg.MODEL.OUT_PLANES,
                                           vit_backbone=self.cfg.MAE.MODEL),
                                   self.cfg,self.device,self.rank,find_unused_parameters=True)
        print('number of parameters: ', sum([p.data.nelement() for p in self.model.parameters()]))
        if self.mode == 'train':
            self.optimizer = build_optimizer(self.cfg, self.model)
            self.lr_scheduler = build_lr_scheduler(self.cfg, self.optimizer)
    ...
    def test(self):
        ...
        print(os.path.join(self.output_dir, 'result_xy.tif'))
        self.model.eval() if self.cfg.INFERENCE.DO_EVAL else self.model.train()
        ...
        if self.output_dir is None:
            ...
            # pdb.set_trace()
            print(os.path.join(self.output_dir, 'result_xy.tif'))
            tifffile.imsave(os.path.join(self.output_dir, 'result_xy.tif'), result[0].astype(np.float32))
    ...
    def build_encoder(self):
        model = mae_vit_base_patch16(norm_pix_loss=True)
        # checkpoint = torch.load('/ygs/personal/chengao/mae-main/output_dir/8_224_224_0.9/abslr/checkpoint-999.pth', map_location='cpu')
        checkpoint = torch.load('/ygs/personal/chengao/connectomics_project/mae_model/test/checkpoint_400000.pth.tar',
                                map_location='cpu')
        model.load_state_dict(checkpoint['state_dict'])
        print('load_model')
